...
public class CsvQueryProcessor implements QueryProcessingEngine {
	...
	public DataSet getResultSet(QueryParameter queryParameter) throws Exception {
		...
		/*
		 * initialize reader to read from the file which is mentioned in
		...
		while ((line = reader.readLine()) != null) {
			...
			ArrayList<Boolean> boolsArray = new ArrayList<Boolean>();
			if(queryParameter.getRestrictions()==null)
				...
				for(int i=0; i<queryParameter.getRestrictions().size();i++) {
					...
					boolsArray.add(filter.evaluateExpression(queryParameter.getRestrictions().get(i), rowFields[index].trim(), rowDataTypeDefinitionMap.get(index)));
				}
				result = solveOperator(boolsArray, queryParameter.getLogicalOperators());
			}
			if(result) {
				Row rowMap = new Row();
	...
	private boolean solveOperator(ArrayList<Boolean> bools, List<String> operators) {
		if(bools.size()==1) {
			return bools.get(0);
